...
const OUT_FILE_TOTS = path.join(OUT_INDEX_DIR, "index-tots-batches.json");
...
async function buildBatches() {
    ...
    const totsCollections = fetchedCollections
        .filter(
            (c) =>
                c.name.toLowerCase().includes("tots") ||
                c.name.toLowerCase().includes("team of the season")
        )
        .map((c) => ({
            id: c.id,
            name: c.name,
    ...
    const otherCollections = fetchedCollections
        .filter(
            (c) =>
                !totwCollections.some((t) => t.id === c.id) &&
                !totsCollections.some((t) => t.id === c.id)
        )
        .map((c) => ({
            id: c.id,
            name: c.name,
    ...
    fs.writeFile(OUT_FILE_TOTS, JSON.stringify(totsCollections, null, 2));
    ...
    const allCollections = [...totwCollections, ...totsCollections, ...otherCollections];
